...
function [e_bi,e_new,e_sec,e_reg,e_fix,max_i] = cal(f,g,fprime,a,b,x0,x1,maxIterations,tolerance,epsilon)
  ...
  disp(f);
  disp("Iteraciones necesitadas con biseccion:");
  disp(l1);
  disp("root:");
  disp(root_bi);
  ...
  disp("Iteraciones necesitadas con regula falsi:");
  disp(l4);
  disp("root:");
  disp(root_reg);
  
  disp("Iteraciones necesitadas con punto fijo:");
  disp(l4);
  disp("root:");
  disp(root_fix);
...
%
%# F1
%figure('Renderer', 'painters', 'Position', [100 500 1100 700]);
%
%x = -100:100;
%
%subplot(2,3,1);
